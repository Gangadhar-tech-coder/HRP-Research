...
# CONFIGURATION
# ============================================
DATASET_PATH = "version2/501_x_2.6k_dataset.csv"  # Update this path to your dataset
...
output_folder = os.path.join(os.getcwd(), f"{dataset_name}")
...
if non_numeric_cols:
    print(f"\nFound {len(non_numeric_cols)} non-numeric column(s): {non_numeric_cols}")
    print(f"Automatically removing: {', '.join(non_numeric_cols)}")
    df = df.select_dtypes(include=[np.number])
else:
    print("All columns are numeric.")

# CRITICAL CHECK: Ensure we have numeric features
if df.shape[1] == 0:
    print("\n" + "="*80)
    print("ERROR: NO NUMERIC FEATURES FOUND!")
    print("="*80)
    print("\nThe dataset contains only non-numeric (text/categorical) columns.")
    print("Please ensure your dataset has numeric columns for dimensionality reduction.")
    print("\nOptions:")
    print("1. Use a different dataset with numeric features")
    print("2. Encode categorical columns to numeric (one-hot encoding, label encoding)")
    print("3. Check if the CSV is properly formatted")
    print("\nExiting program.")
    exit(1)

# Check for NaN values and handle them
if df.isnull().any().any():
    print("\nWarning: Found NaN/missing values in the dataset.")
...
if use_subset == 'y':
    ...
    elif n_samples_to_use < 10:
        print(f"Warning: {n_samples_to_use} samples is too few. Using minimum of 10 samples.")
        n_samples_to_use = min(10, df.shape[0])
...
if X.shape[1] < 2:
    ...
    print("="*80)
    print(f"Found only {X.shape[1]} feature(s).")
    print("Dimensionality reduction requires at least 2 features.")
...
for reduction_percentage in REDUCTION_PERCENTAGES:
    ...
    if k < 1:
        print(f"\nSkipping reduction {reduction_percentage}% (k={k} too small)")
        continue
    ...
    print(f"{'-'*80}")
    print(f"Best Alpha for {reduction_percentage}% Reduction: {best_alpha:.1f}")
...
# Check if any experiments were completed
if not best_alphas:
    print("\n" + "="*80)
    print("WARNING: NO EXPERIMENTS COMPLETED!")
    print("="*80)
    print("All reduction percentages resulted in k < 1.")
    print("This means your dataset has too few features for the requested reductions.")
    print(f"Current features: {n_features}")
    print(f"Minimum features needed: At least 10 for 10% reduction")
    
    # Save minimal Excel file
    writer.close()
    workbook = load_workbook(excel_path)
    format_excel_sheet(workbook['Dataset Info'], has_header=True)
    workbook.save(excel_path)
    
    print(f"\nDataset info saved to: {excel_path}")
    print("Exiting program.")
    exit(0)
...
with open(readme_path, 'w', encoding='utf-8') as f:
    ...
    f.write(f"Dimensions: {n_samples} samples x {n_features} features\n\n")
    f.write("="*80 + "\n")
    f.write("FILES IN THIS FOLDER:\n")
    f.write("="*80 + "\n")
    f.write(f"1. {dataset_name}_results.xlsx - Complete results in Excel format\n")
    f.write(f"2. images/ - All generated graphs ({len(best_alphas) + 1} images)\n")
    ...
    for reduction_percentage in REDUCTION_PERCENTAGES:
        if reduction_percentage in best_alphas:
            f.write(f"- Reduction_{reduction_percentage}pct: Alpha vs Distortion table (NRP, PMRP, HRP)\n")
    f.write("\n")
    f.write("="*80 + "\n")
...
print(f"\nExperiments completed: {len(best_alphas)} reduction(s)")
